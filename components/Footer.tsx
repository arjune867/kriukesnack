...
const Footer: React.FC<FooterProps> = ({ navigate, currentPage }) => {
    const navItems = [
        ...
        { page: 'promo' as Page, label: 'Promo', icon: 'ticket' as const },
        { page: 'cart' as Page, label: 'Keranjang', icon: 'cart' as const },
        { page: 'profile' as Page, label: 'Profil', icon: 'user' as const },
    ];

    return (
        <footer className="bg-white border-t border-gray-200 dark:bg-gray-800 dark:border-gray-700 fixed bottom-0 left-0 right-0 max-w-md mx-auto h-16 z-40">
            <nav className="h-full">
                <ul className="flex justify-around items-center h-full">
                    {navItems.map((item) => {
                        const isActive = currentPage === item.page;
                        const colorClass = isActive ? 'text-orange-500' : 'text-gray-400 dark:text-gray-500';
                        return (
                            <li key={item.page} className="flex-1">
                                ...
                                >
                                    <Icon name={item.icon} isSolid={isActive && (item.icon === 'home' || item.icon === 'heart' || item.icon === 'user' || item.icon === 'ticket')} />
